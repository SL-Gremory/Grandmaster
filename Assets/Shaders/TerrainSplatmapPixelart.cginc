...
#endif
{
	...
	//splatUV -= splatUV % (_Control_TexelSize.xy); // WOW this lines up with the actual game tiles
	splatUV -= splatUV % (_Control_TexelSize.xy / (32.0)); // per-pixel, adjust for texture resolution
	...
	mixedDiffuse = 0.5;
	float densTarget = (splat_control.r + splat_control.g + splat_control.b + splat_control.a) / 4.0;
#ifdef TERRAIN_STANDARD_SHADER
	if (splat_control.r >= densTarget)
		mixedDiffuse = tex2D(_Splat0, uvSplat0) * half4(1.0, 1.0, 1.0, defaultAlpha.r);
	if (splat_control.g > densTarget)
		mixedDiffuse = tex2D(_Splat1, uvSplat1) * half4(1.0, 1.0, 1.0, defaultAlpha.g);
	if (splat_control.b > densTarget)
		mixedDiffuse = tex2D(_Splat2, uvSplat2) * half4(1.0, 1.0, 1.0, defaultAlpha.b);
