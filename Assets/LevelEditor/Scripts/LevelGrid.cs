...
public class LevelGrid : MonoBehaviour
{
	...
	void ProcessTerrain(TerrainData data)
	{
		...
		if (walkables == null || walkables.Length != sizeX * sizeZ)
			walkables = new Walkability[sizeX * sizeZ];
		...
		for (int x = 0; x < sizeX; x++)
		{
			...
			{
				var prop = HasPropThere(x, z, data);
				if (prop != PropPresence.Prop)
				{
					if (!useTerrain)
						continue;
					heights[x + sizeX * z] = data.GetInterpolatedHeight((x + 0.5f) / (float)sizeX, (z + 0.5f) / (float)sizeZ);
					if (prop != PropPresence.OnlyWalkability)
					{
						SampleWalkability(x, z, data);
					}
					if (setUnwalkableAngle > 0.001f)
					{
						walkables[x + sizeX * z] = data.GetSteepness((x + 0.5f) / (float)sizeX, (z + 0.5f) / (float)sizeZ) < setUnwalkableAngle ? walkables[x + sizeX * z] : Walkability.Unwalkable;
					}
				}
			}
		}
	...
	void SampleWalkability(int x, int z, TerrainData data)
	{
		var layers = data.GetAlphamaps(x, z, 1, 1);
		int mostIndex = 0;
		for (int i = 0; i < layers.GetLength(2); i++)
		{
			if (layers[0, 0, i] > layers[0, 0, mostIndex])
				mostIndex = i;
		}
		var layer = data.terrainLayers[mostIndex];
		if (terrainTypes == null)
			return;
		var walk = terrainTypes.FindWalkability(layer);
		walkables[x + sizeX * z] = walk;
	}
	...
	enum PropPresence
	{
		None, Prop, OnlyWalkability
	}

	PropPresence HasPropThere(int posX, int posZ, TerrainData data)
	{
		if (!useProps)
			return PropPresence.None;
		...
		for (int i = 0; i < hits.Length; i++)
		{
			...
			if (prop != null)
			{
				if (prop.OnlyWalkability())
				{
					SetWalkable(posX, posZ, prop.GetWalkability());
					return PropPresence.OnlyWalkability;
				}
				SetWalkable(posX, posZ, prop.GetWalkability());
				SetHeight(posX, posZ, hit.point.y - transform.position.y);
				return PropPresence.Prop;
			}
		}
		return PropPresence.None;
	}
	...
	/// <param name="z"></param>
	/// <returns></returns>
	public Walkability GetWalkability(int x, int z)
	{
		Debug.Assert(x >= 0 && z >= 0 && x < sizeX && z < sizeZ);
	...
	public void SetWalkable(int x, int z, Walkability value)
	{
		Debug.Assert(x >= 0 && z >= 0 && x < sizeX && z < sizeZ);
